...
function create_directories () {
    local bolt_path=$(influxd print-config --key-name bolt-path ${@})
    local bolt_dir=$(dirname ${bolt_path})
    local engine_path=$(influxd print-config --key-name engine-path ${@})
    local user=$(id -u)
...
function upgrade_influxd () {
    local log_level=$(influxd print-config --key-name log-level ${@})
    local bolt_path=$(influxd print-config --key-name bolt-path ${@})
    local engine_path=$(influxd print-config --key-name engine-path ${@})
...
function main () {
    ...
    if [ $# -eq 0 -o "$(echo "$1" | cut -c1-1)" = '-' ]; then
        # No command given, assume influxd.
        influxd_main ${@}
    elif [ "$1" = 'influxd' ] && [ $# -eq 1 -o "$(echo "$2" | cut -c1-1)" = '-' ]; then
