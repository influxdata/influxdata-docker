...
#
# The image is built by the root script as tag "telegraf:1.39-distroless"
# (path telegraf/1.39/distroless -> first '/' becomes ':', the rest become '-').

tag="telegraf:1.39-distroless"
...
assert_contains "${version_output}" "1.39.0"   "${tag} --version reports 1.39.0"
