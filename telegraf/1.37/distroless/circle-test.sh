...
#
# The image is built by the root script as tag "telegraf:1.37-distroless"
# (path telegraf/1.37/distroless -> first '/' becomes ':', the rest become '-').

tag="telegraf:1.37-distroless"
...
assert_contains "${version_output}" "1.37.3"   "${tag} --version reports 1.37.3"
